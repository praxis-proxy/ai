...
mod tests {
    ...
    fn the_policy_names_the_key_and_rejects_by_default() {
        ...
        assert_eq!(
            policy.pointer("/default/0/type").and_then(serde_json::Value::as_str),
            Some("reject"),
            "everything else is rejected"
        );
        let rule = policy
            .pointer(&format!("/transports/docker/{REGISTRY}/0"))
            .expect("a rule for Red Hat's registry");
        assert_eq!(
            rule.get("type").and_then(serde_json::Value::as_str),
            Some("signedBy"),
            "Red Hat's registry requires a signature"
        );
        assert_eq!(
            rule.get("keyPath").and_then(serde_json::Value::as_str),
            Some("/tmp/key.asc"),
            "signed by the bundled key"
        );
    }
}
