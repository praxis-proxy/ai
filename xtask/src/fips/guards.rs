...
/// The crates that make up the shipped binary, relative to the workspace root.
const PRODUCT_CRATES: &[&str] = &["apis", "filters", "server", "integrations"];
...
const GUARDS: &[Guard] = &[
    ...
    Guard {
        ...
        pattern: r#"^[^#]*("vendored"|openssl-src)"#,
        ...
        paths: &["Cargo.toml", "apis", "filters", "server", "integrations"],
    },
    Guard {
...
mod tests {
    ...
    /// The guard with this label.
    fn guard(label: &str) -> &'static Guard {
        GUARDS
            .iter()
            .find(|guard| guard.label == label)
            .expect("a guard with that label")
    }

    #[test]
    fn guard_patterns_compile_and_match_what_they_should() {
        let vendored = Regex::new(guard("vendored OpenSSL").pattern).expect("valid");
        assert!(
        ...
        assert!(
            !vendored.is_match(r#"prost-wkt-types = { version = "0.7.2", features = ["vendored-protox"] }"#),
            "vendoring something other than OpenSSL is fine"
        );
        assert!(
            vendored.is_match(r#"openssl-src = "300""#),
            "the vendored OpenSSL source crate itself"
        );
        let enable = Regex::new(guard("application enables a FIPS provider itself").pattern).expect("valid");
        assert!(
