...
fn missing_libcrypto() -> Finding {
    Finding {
        ...
        why: "no cryptography can be reaching the RHEL OpenSSL FIPS module; the binary carries its own crypto instead \
            ...
            .to_owned(),
        location: "praxis-tls (praxis_tls::provider) is the only place a provider is chosen; a binary without \
                   libcrypto was built without it (or against a vendored OpenSSL)"
            .to_owned(),
...
mod tests {
    ...
    fn the_test_binary_itself_is_a_rust_elf_and_the_symbol_scan_sees_what_it_links() {
        ...
        if cfg!(feature = "dev") {
            ...
            // policy engine (and reqwest) bundle aws-lc; the scan must see it.
            assert!(
