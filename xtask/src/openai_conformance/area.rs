...
pub(super) const CONFORMANCE_AREAS: &[ApiArea] = &[ApiArea {
    ...
    runtime_test_command: "cargo test -p praxis-ai-apis --no-default-features --features store-all,callout-rustls --lib conformance_conversations_ -- --show-output",
    runtime_test_args: &[
        "test",
        "-p",
        "praxis-ai-apis",
