...
| [token-rate-limit-per-app.yaml](configs/token-rate-limit-per-app.yaml) | Extends token-rate-limit.yaml with ai#129's bucket_key_header: each unique value of the configured request header (here, x-app-id) gets its own independent token bucket, so a platform admin can hand out per-app budgets from a single filter instance |
