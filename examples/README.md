...
| [intelligent-route-claim-gating.yaml](configs/intelligent-route-claim-gating.yaml) | Fences candidates by the caller's data-residency region. `match_claims` reads the `grid_region` claim off the authenticated identity and keeps only candidates whose `region` label equals it, before the model-name pick runs on the survivors |
