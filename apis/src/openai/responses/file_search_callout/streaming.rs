        ...
        Err(_) => match on_failure {
            OnFailure::Closed => {
                fs_end_stream_with_error(
                    ...
                    state,
                    "server_busy",
                    "openai_file_search_callout: file_search admission overloaded",
                );
...
pub(super) fn step_0_5_translate_and_mixed_tool(
    ...
    if mixed {
        fs_end_stream_with_error(
            ...
            state,
            "server_error",
            "openai_file_search_callout: response mixes file_search with a client tool call",
        );
        return Err(());
    }
    Ok(translated)
...
pub(super) fn branch_b_terminal(
    ...
    if annotate_output_items(&mut state.accumulated_output, &state.citation_files).is_err() {
        fs_end_stream_with_error(
            ...
            state,
            "server_error",
            "openai_file_search_callout: citation annotation failed",
        );
    ...
    if !terminal_payload_fits(state, max_json_body_bytes) {
        fs_end_stream_with_error(
            ...
            state,
            "server_error",
            "openai_file_search_callout: terminal response exceeds byte limit",
        );
        return Ok(FilterAction::Continue);
    }
