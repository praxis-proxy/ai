...
fn mixed_client_and_file_search_calls_are_rejected_before_search() {
    ...
    assert!(matches!(
        FileSearchDispatchFilter::capture_response(&mut ctx, &mut body, 268_435_456).unwrap(),
        FilterAction::Reject(_)
    ));
...
fn exhausted_tool_budget_finishes_without_another_inference() {
    ...
    assert_eq!(
        ctx.filter_results["openai_file_search_dispatch"].get("pending"),
        Some("false")
    );
}
...
fn translate_skips_without_tool_declaration() {
    ...
    let _action = FileSearchDispatchFilter::capture_response(&mut ctx, &mut body, 268_435_456).unwrap();
...
fn translate_triggers_pending_flag() {
    ...
    let action = FileSearchDispatchFilter::capture_response(&mut ctx, &mut body, 268_435_456).unwrap();
...
async fn branch_a_reconciles_round_and_continues() {
    ...
    let r = ctx.filter_results.get("openai_file_search_dispatch").unwrap();
