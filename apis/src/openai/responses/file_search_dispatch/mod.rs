...
impl FileSearchDispatchFilter {
    ...
    fn apply_batch(state: &mut ResponsesState, plan: &SearchPlan, batch: &SearchBatch) -> Result<(), FilterAction> {
        ...
        if !response_fits(state, MAX_JSON_BODY_BYTES) {
            return Err(FilterAction::Reject(responses_error_rejection(
                502,
                "server_error",
                "openai_file_search_dispatch: continuation output exceeds the JSON response byte limit",
            )));
        }
        state
            .messages
    ...
    fn failure_rejection(&self, batch: &SearchBatch) -> Option<FilterAction> {
        ...
        Some(FilterAction::Reject(responses_error_rejection(
            502,
            "server_error",
            &format!("openai_file_search_dispatch: {}", failure.error),
        )))
    }
    ...
    async fn execute_pending(&self, ctx: &mut HttpFilterContext<'_>) -> Result<FilterAction, FilterError> {
        ...
        let state = ctx
            ...
            .ok_or_else(|| -> FilterError { "openai_file_search_dispatch: ResponsesState disappeared".into() })?;
        ...
        if plan.has_pending_calls {
            ...
            if self.failure_rejection(&batch).is_some() {
                streaming::fs_end_stream_with_error(
                    ...
                    state,
                    "server_error",
                    "openai_file_search_dispatch: search failed",
                );
                return Ok(FilterAction::Continue);
            }
            let framework_bytes = retained_iteration_bytes(ctx);
            if Self::apply_batch(state, &plan, &batch).is_err() {
                streaming::fs_end_stream_with_error(
                    ...
                    state,
                    "server_error",
                    "openai_file_search_dispatch: apply_batch failed",
                );
                return Ok(FilterAction::Continue);
            }
            ...
            if !continuation_state_fits(framework_bytes, state, self.max_state_bytes, 0) {
                streaming::fs_end_stream_with_error(
                    ...
                    state,
                    "server_error",
                    "openai_file_search_dispatch: continuation state too large",
                );
                return Ok(FilterAction::Continue);
            }
            reset_tool_choice(state);
            state.iteration = state.iteration.saturating_add(1);
            let results = ctx.filter_results.entry("openai_file_search_dispatch").or_default();
    ...
    fn capture_response(
        ...
        if !combined_output_fits(state, &response, MAX_JSON_BODY_BYTES) {
            return Ok(FilterAction::Reject(responses_error_rejection(
                502,
                "server_error",
                "openai_file_search_dispatch: accumulated output exceeds the JSON response byte limit",
            )));
        }
...
impl HttpFilter for FileSearchDispatchFilter {
    ...
    fn response_body_mode(&self) -> BodyMode {
        ...
        // an iterative-router step with `openai_proxy`, which always
        ...
        if ctx.subrequest_response_mode() == SubRequestResponseMode::Streaming {
            ...
            if !armed {
                return Ok(FilterAction::Reject(responses_error_rejection(
                    500,
                    "server_error",
                    "openai_file_search_dispatch: logical stream not armed by openai_stream_events",
                )));
            }
        }
...
fn continuation_state_rejection() -> FilterAction {
    FilterAction::Reject(responses_error_rejection(
        ...
        "openai_file_search_dispatch: continuation state exceeds max_state_bytes",
    ))
}
...
fn invalid_success_response_action(continued: bool) -> FilterAction {
    if continued {
        FilterAction::Reject(responses_error_rejection(
            502,
            "server_error",
            "openai_file_search_dispatch: inference continuation returned an invalid response",
        ))
    } else {
...
fn finalize_public_response(state: &mut ResponsesState) -> Result<Bytes, FilterAction> {
    ...
    *state.output_items_mut() = combined_output;
    ...
    annotate_response(&mut state.response_object, &state.citation_files).map_err(|error| {
        ...
        final_response_rejection("openai_file_search_dispatch: failed to annotate final response")
    })?;
    bounded_json_size(&state.response_object, MAX_JSON_BODY_BYTES)
        .ok()
        .flatten()
        .ok_or_else(|| {
            final_response_rejection("openai_file_search_dispatch: final response exceeds the JSON response byte limit")
        })?;
    serde_json::to_vec(&state.response_object)
        .map(Bytes::from)
        .map_err(|error| {
            warn!(%error, "failed to encode final file-search response");
            final_response_rejection("openai_file_search_dispatch: failed to encode final response")
        })
}
...
fn mixed_tool_response_rejection() -> FilterAction {
    FilterAction::Reject(responses_error_rejection(
        502,
        "server_error",
        "openai_file_search_dispatch: a model response cannot combine file_search_call with client-executed function_call",
    ))
}
