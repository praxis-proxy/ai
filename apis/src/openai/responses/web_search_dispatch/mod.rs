...
//!    and writes `openai_web_search_dispatch.action = "loop"` to [`filter_results`] for the IRR step transition.
...
//! - The IRR transition must match `openai_web_search_dispatch.action = "loop"` and target the same inference step.
//!
...
const FILTER_RESULT_KEY: &str = "openai_web_search_dispatch";
...
/// mirroring
/// [`remaining_file_search_call_budget`](super::file_search_dispatch).
