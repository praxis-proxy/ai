...
///
/// The pipeline contains only the internal upstream-seed filter, which the
/// production path prepends to every file-resolve `outbound_chain`
/// (`seed_file_resolve_chain`): the configured Files API destination is owned
/// by the API client, so without the seed filter the executor has no upstream
/// to dial. SSRF policy (`allow_private_upstreams`) is what these resolution
/// tests otherwise exercise.
fn private_outbound_pipeline() -> Arc<FilterPipeline> {
    ...
    praxis_filter::register_filters!(
        @register registry,
        http "openai_callout_seed_upstream" => crate::openai::CalloutSeedUpstreamFilter::from_config
    );
    let mut entries = [FilterEntry {
