...
use self::config::{ChatCompletionsToAzureaiChatCompletionsConfig, build_config};
use crate::{
    azure::wire,
...
const RESPONSE_TRANSFORM_KEY: &str = "azureai_translation.response_transform";
...
/// ```
pub struct ChatCompletionsToAzureaiChatCompletionsFilter {
    /// Parsed and validated configuration.
    config: ChatCompletionsToAzureaiChatCompletionsConfig,
}

impl ChatCompletionsToAzureaiChatCompletionsFilter {
    ...
    pub fn from_config(config: &serde_yaml::Value) -> Result<Box<dyn HttpFilter>, FilterError> {
        let cfg: ChatCompletionsToAzureaiChatCompletionsConfig =
            parse_filter_config("openai_chat_completions_to_azureai_chat_completions", config)?;
        let validated = build_config(cfg)?;
        Ok(Box::new(ChatCompletionsToAzureaiChatCompletionsFilter {
            config: validated,
        }))
    }
}
...
#[async_trait]
impl HttpFilter for ChatCompletionsToAzureaiChatCompletionsFilter {
...
mod tests {
    ...
    fn default_config_parses() {
        ...
        assert_eq!(filter.name(), "openai_chat_completions_to_azureai_chat_completions");
    }
    ...
    fn unknown_config_field_rejected() {
        ...
        let result = ChatCompletionsToAzureaiChatCompletionsFilter::from_config(&yaml);
    ...
    #[test]
    fn rebuild_sse_frames_preserves_event_type() {
        let data = br#"{"id":"chatcmpl-abc","choices":[{"delta":{"content":"Hi"}}]}"#;
        let frames = vec![SseFrame {
