...
use praxis_test_utils::{free_port, http_post, start_backend, start_header_echo_backend, start_proxy};
...
#[test]
fn grid_credential_inject_adds_selected_bearer_token() {
    let backend = start_header_echo_backend();
    let proxy_port = free_port();

    let yaml = make_credential_inject_yaml(proxy_port, backend.port());
    let config = Config::from_yaml(&yaml).unwrap();
    let proxy = start_proxy(&config);
    ...
    let (status, body) = http_post(
        proxy.addr(),
        "/v1/chat/completions",
        r#"{"model":"hosted-api-model","messages":[]}"#,
    );
    assert_eq!(status, 200, "credential-bearing model should route");
    assert!(
        body.lines()
            .any(|line| line.eq_ignore_ascii_case("authorization: Bearer example-token")),
        "backend should receive injected Authorization header; got:\n{body}"
    );
}
...
/// Build YAML config that mirrors the grid-credential-inject.yaml example
/// with dynamic ports substituted in.
fn make_credential_inject_yaml(proxy_port: u16, backend_port: u16) -> String {
    format!(
        r#"
listeners:
  - name: proxy
