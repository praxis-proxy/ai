...
fn load_file_search_dispatch_config(proxy_port: u16, port_map: &HashMap<&str, u16>) -> Config {
...
#[test]
fn file_search_dispatch_example_runs_model_search_model_round_trip() {
    ...
    let config = load_file_search_dispatch_config(
        proxy_port,
...
#[test]
fn file_search_dispatch_example_without_tools_passthrough() {
    ...
    let config = load_file_search_dispatch_config(proxy_port, &HashMap::from([("127.0.0.1:3001", backend.port())]));
    let proxy = start_file_search_proxy(&config);
...
// #313 §7.1: the buffered file-search-dispatch example fails closed on `stream:true`.
// The old `unsupported_streaming_rejection` (400) was removed; the proxy now auto-derives
// the streaming transport from the client's `stream:true`, so `openai_file_search_dispatch`
...
#[test]
fn file_search_dispatch_example_fails_closed_on_unarmed_streaming() {
...
#[test]
fn file_search_dispatch_example_rejects_parallel_client_function_call() {
...
#[test]
fn file_search_dispatch_example_rejects_non_success_search_response() {
    let first_model_response = json!({
        "id": "resp_search",
        "object": "response",
        "status": "completed",
