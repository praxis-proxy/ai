...
Finite requests using `previous_response_id` require `openai_response_store` and `openai_responses_rehydrate` earlier in the request pipeline. The filter fails closed if stored history has not been resolved, preventing a continuation from silently losing prior turns. Streaming continuation translation is tracked separately in [issue #36](https://github.com/praxis-proxy/ai/issues/36).
