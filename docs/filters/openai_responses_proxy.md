...
Non-null `prompt` template references are rejected unless the load balancer selected a cluster declaring `application_protocol: openai_responses` and `application_provider: openai`. The check runs in the selected-upstream body phase, after routing has frozen that application metadata. Missing or different application metadata fails closed.
