...
Reads the assembled conversation history from `ResponsesState::messages` and replaces the `input` field in the outbound body when it differs from the original normalized input. For a provider-owned `conversation`, internal agentic-loop continuations send only items not already persisted by the provider. Strips `previous_response_id` after Praxis resolves it locally via the rehydrate filter.
