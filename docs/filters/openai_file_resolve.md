...
| `max_resolved_bytes` | integer | no | Maximum size in bytes of a single resolved file's content fetched from the Files API or a remote URL (default 64 MiB). Bounds each individual callout payload before it is inlined, independently of the total rewritten body size. |
