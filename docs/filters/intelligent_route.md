...
| `skip_paths` | string[] | no | Request-path prefixes that bypass model resolution entirely. Management and discovery endpoints (model listing, subscriptions, API-key management, health) carry no routable model, yet a stray body `model` field promoted into the model header would otherwise fail the request closed (404) or misroute it. Requests whose path matches one of these prefixes return `Continue` before any model lookup, leaving the cluster untouched for a downstream path router to serve. Matched with Gateway-API segment-boundary prefix semantics against the request path (a trailing slash on the prefix is ignored). Defaults to the well-known OpenAI-style management paths; set an explicit list to override, or `[]` to disable path skipping entirely. |
| `management_cluster` | string | no | Cluster that serves skipped management/discovery paths. When set, a request whose path matches `skip_paths` has `ctx.cluster` set to this cluster and continues straight to `load_balancer`. When unset, skipped paths continue with the cluster untouched. Requires a non-empty `skip_paths`. |
