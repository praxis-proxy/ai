...
The histogram's `model` label is read from metadata set by an upstream format filter (`openai_format`, `anthropic_messages_format`, or `anthropic_messages_to_chat_completions`). If no format filter runs before this filter, all TTFT samples are labeled `unknown`.
